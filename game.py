...
class Level1: 
    def __init__(self, notifier):
        ...
        self.words = ["casa", "perro", "libro", "mesa", "gato", "palo"]
        self.setup_level()

    def setup_level(self):
        self.notifier.notify({"type": "speak", "text": "Nivel 1: Completa las palabras con sílabas"})
        ...
        all_syllables = self.syllables + ["la", "lo", "pa", "sa", "ti"]
    ...
    def update(self):
        if not self.completed:
            ...
            if all_correct:
                ...
                self.notifier.notify({"type": "speak", "text": "Palabra incorrecta. Intenta de nuevo."})
                for space in self.spaces:
...
class Level2:  
    def __init__(self, notifier):
        self.notifier = notifier
        self.words = ["escuela", "familia", "maestro", "juguete"]
        self.setup_level()

    def setup_level(self):
        self.notifier.notify({"type": "speak", "text": "Nivel 2: Ordena las sílabas para formar la palabra"})
        self.word = random.choice(self.words)
        self.syllables = self._split_syllables(self.word)
        ...
        if not self.completed:
            ...
            if all_filled:
                ...
                if formed_word == self.word:
                    ...
                    self.error_message = "¡Palabra incorrecta! Inténtalo de nuevo."
                    self.error_timer = 180
                    self.notifier.notify({"type": "speak", "text": "Palabra incorrecta, inténtalo de nuevo"})
    ...
    def handle_event(self, event):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if event.button == 1:
                if 450 <= event.pos[0] <= 550 and 400 <= event.pos[1] <= 440:
                    all_filled = all(space.occupied for space in self.spaces)
                    if all_filled:
                        formed_word = ""
                        for space in self.spaces:
